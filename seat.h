...
struct Bus {
    int unqID;    //"unqID" in bus and "id" in Passenger are same..
    struct Passenger *firstPassenger;
};
char ticid[50];
struct Bus *arr[BUS_NUM];    //array of pointers of struct Bus datatype
		...
		switch(ch){
			case 1:
				...
				if(n<=5){
					...
					printf("The cost of tickets id: %.2f",price1(from,to,n));
				}
				else{
					printf("Total number of seats should be less than 5!!");
				}
				break;
			case 2:
				display();
